...
namespace CopyRestAPI.Helpers
{
    public class AuthorizationHeader
    {
        ...
        private Dictionary<string, string> _parameters;
                                           //TODO: Fix, cannot initialized at the top. The ordering is messed up

        private string OauthConsumerSecret { get; set; }
        private string OauthTokenSecret { get; set; }

        public string CreateForRequest(string callbackUrl, string consumerKey, string consumerSecret, string url)
        {
            ...
            _parameters.Remove("oauth_token");
            _parameters.Remove("oauth_verifier");

            _parameters["oauth_callback"] = callbackUrl;
            ...
            _parameters["oauth_signature"] = GenerateSignature(url, "GET");

            return "OAuth " + EncodeRequestParameters();
        }

        public string CreateForAccess(string consumerKey, string consumerSecret, string token, string tokenSecret,
                                      string verifier)
        {
            _parameters = new Dictionary<string, string>
                {
            ...
            _parameters.Remove("oauth_callback");
            ...
            _parameters["oauth_signature"] = GenerateSignature(Consts.AccessToken, "GET");

            return "OAuth " + EncodeRequestParameters();
        }

        public string CreateForREST(string consumerKey, string consumerSecret, string token, string tokenSecret,
                                    string url, string method)
        {
            ...
            _parameters["oauth_timestamp"] = GenerateTimeStamp();
            _parameters["oauth_token"] = token;
            _parameters["oauth_signature"] = GenerateSignature(url, method);
        ...
        public string GenerateSignature(string uri, string method)
        {
        ...
        private string GetSignatureBase(string url, string method)
        {
            ...
            foreach (KeyValuePair<String, String> item in _parameters.Where(x => x.Key != "oauth_signature"))
            {
        ...
        {
            string keystring = string.Format("{0}&{1}", UrlEncode(OauthConsumerSecret),
                                             UrlEncode(OauthTokenSecret ?? string.Empty));
            return new HMACSHA1(Encoding.UTF8.GetBytes(keystring));
        }
